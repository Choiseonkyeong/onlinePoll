...
//	텍스트 파일이 저장된 경로와 이름을 넘겨받아 텍스트 파일에 저장된 데이터를 읽어서 ArrayList에 저장시킨후 
//	리턴하는 메소드
	public static ArrayList<String> pollRead(String filepath) {
		
//		읽어들인 텍스트 파일의 데이터를 저장해 리턴 시킬 ArrayList를 선언한다.
		ArrayList<String> poll = null;
//		텍스트 파일에서 데이터를 읽어들일 Scanner를 선언한다.
		Scanner scanner = null;
		try {
//			텍스트 파일에서 데이터를 읽어들인 Scanner 객체를 생성한다.
			scanner =new Scanner(new File(filepath));
//			스캐너로 읽어들인 텍스트 파일 객체가 정상적으로 생성되면 데이터를 저장해 리턴하는 ArrayList 객체를 생성한다.
