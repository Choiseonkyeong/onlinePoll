...
//		 넘어온 투표 데이터가 숫자인가 검사한다
	...
	//		넘어온 투표 데이터가 숫자인 경우 정상적인 투표 범위의 데이터인가 검사하기 위해서 텍스트 파일을 읽어서 투표 항목의 
	//		개수를 계산한다.
	...
	//			여기까지 왔다는 것은 정상적인 투표 데이터가 넘어왔다는 것이므로 투표한 항목의 득표수를 증가 시킨다.
	//			득표수를 1증가 시킬 위치를 계산한다.
				int index =result+itemCount;
	//			득표수를 증가시킨다. => ArrayList의 index번째 데이터를 1증가시킨다.
...
//		jsp는 서버용 스크립트 언어이고 javascrip는 클라이언트용 스크립트 언어이다.
//		하나의 jsp 파일에 서버용 스크립트와 클라이언트용 스크립트를 모두 사용한 경우 서버용 스크립트가 먼저 다 실행되고 난 후 클라이언트용 
