...
<%
//	프로젝트를 실행하면 이클립스가 자체적으로 웹 서버를 구축하고 구축된 웹 서버에서 프로젝트를 실행한다. => 실제경로(realPath)
//  이클립스에서 웹 서비스가  구현되는 실제경로
//	application.getRealPath("/") 명령을 실행해서 이클립스에서 웹서비스가 구현되는 실제 경로를 얻어 올 수 있다.
//	out.println(application.getRealPath("/"));		// "/"는 웹 서비스가 실행되는 web root(최상위 경로 )
...
<form action="pollWrite.jsp" method="post">
		...
		<!-- cellspacing: 셀과 셀 사이의 간격을 지정한다. => 셀 간격    -->
		...
		<!-- 투표 항목의 개수만큼 반복하며 라디오 버튼과 투표항목을 출력한다.  -->
		<% 
